...
var _rename_pending_requests := {}
var _rename_initialized := false
var _rename_queued := {}
...
var _inline_tcp := StreamPeerTCP.new()
var _inline_read_buffer := PackedByteArray()
var _inline_next_id := 1
var _inline_pending_requests := {}
var _inline_initialized := false
var _inline_queued := false
var _inline_last_status := StreamPeerTCP.STATUS_NONE
...
func _rename_process_connection() -> void:
	if _rename_tcp.get_status() == StreamPeerTCP.STATUS_NONE:
		return

	_rename_tcp.poll()
	var status := _rename_tcp.get_status()
	if status != _rename_last_status:
		_debug_rename("TCP status changed to %d." % status)
		_rename_last_status = status

	if status == StreamPeerTCP.STATUS_CONNECTED:
		_rename_read_available_messages()
...
func _rename_send_initialize_request() -> void:
	var root_path := ProjectSettings.globalize_path("res://")
	var root_uri := _path_to_file_uri(root_path)
	var id := _rename_next_request_id()
	_rename_pending_requests[id] = "initialize"
	_debug_rename("sending initialize request.")
...
func _rename_send_initialized_notification() -> void:
	_debug_rename("sending initialized notification.")
	_rename_send_message({
		"jsonrpc": "2.0",
		"method": "initialized",
		"params": {},
...
func _rename_send_text_document_sync_notification(uri: String, text: String) -> bool:
	...
	if not _rename_opened_documents.has(uri):
		_rename_opened_documents[uri] = true
...
func _rename_send_message(message: Dictionary) -> void:
	...
	var err := _rename_tcp.put_data(packet.to_utf8_buffer())
	if err != OK:
		print("Rename Symbol: failed sending request, error %d." % err)
	else:
		_debug_rename("sent %s." % message.get("method", "response"))


func _rename_read_available_messages() -> void:
	var available := _rename_tcp.get_available_bytes()
	if available <= 0:
		return

	_debug_rename("reading %d byte(s)." % available)

	var read_result := _rename_tcp.get_data(available)
	if read_result[0] != OK:
		print("Rename Symbol: failed reading response.")
		return

	_rename_read_buffer.append_array(read_result[1])

	while true:
		var body := _try_extract_lsp_body(_rename_read_buffer)
		if body.is_empty():
			return

		_rename_read_buffer = _consume_lsp_message(_rename_read_buffer)
		var message = JSON.parse_string(body)
		if typeof(message) == TYPE_DICTIONARY:
			_debug_rename("received message %s." % JSON.stringify(message).left(500))
			_rename_handle_message(message)


func _rename_handle_message(message: Dictionary) -> void:
	if not message.has("id"):
		return

	var id = _normalize_response_id(message["id"])
	if not _rename_pending_requests.has(id):
		print("Rename Symbol: received an unexpected response.")
		return

	var request_kind: String = _rename_pending_requests[id]
...
func _rename_reset_connection() -> void:
	_rename_tcp.disconnect_from_host()
	_rename_tcp = StreamPeerTCP.new()
	_rename_read_buffer.clear()
	_rename_pending_requests.clear()
	_rename_initialized = false
...
func _inline_process_connection() -> void:
	if _inline_tcp.get_status() == StreamPeerTCP.STATUS_NONE:
		return

	_inline_tcp.poll()
	var status := _inline_tcp.get_status()
	if status != _inline_last_status:
		_debug_inline("TCP status changed to %d." % status)
		_inline_last_status = status

	if status == StreamPeerTCP.STATUS_CONNECTED:
		_inline_read_available_messages()
		if not _inline_initialized and not _inline_pending_requests.values().has("initialize"):
			_inline_send_initialize_request()
	elif status == StreamPeerTCP.STATUS_ERROR:
		print("Inline Variable: connection error.")
		_inline_reset_connection()
...
func _inline_ensure_connection() -> bool:
	...
	_inline_reset_connection()
	var err := _inline_tcp.connect_to_host(HOST, PORT)
	if err != OK:
		print("Inline Variable: could not connect to the code analysis service, error %d." % err)
		return false

	_debug_inline("connecting to code analysis service at %s:%d..." % [HOST, PORT])
	return true


func _inline_send_initialize_request() -> void:
	var root_path := ProjectSettings.globalize_path("res://")
	var root_uri := _path_to_file_uri(root_path)
	var id := _inline_next_request_id()
	_inline_pending_requests[id] = "initialize"

	_inline_send_message({
		"jsonrpc": "2.0",
		"id": id,
		"method": "initialize",
		"params": {
...
func _inline_send_initialized_notification() -> void:
	_inline_send_message({
		"jsonrpc": "2.0",
		"method": "initialized",
		"params": {},
...
func _inline_try_send_references_request() -> void:
	if not _inline_queued or not _inline_initialized:
		return

	_inline_send_document_sync_notification()

	var id := _inline_next_request_id()
	_inline_pending_requests[id] = "references"
	_inline_send_message({
		"jsonrpc": "2.0",
		"id": id,
...
func _inline_send_document_sync_notification() -> void:
	var version := int(_inline_document_versions.get(_inline_uri, 0)) + 1
	_inline_document_versions[_inline_uri] = version

	if not _inline_opened_documents.has(_inline_uri):
		_inline_opened_documents[_inline_uri] = true
		_inline_send_message({
			"jsonrpc": "2.0",
			"method": "textDocument/didOpen",
			"params": {
				"textDocument": {
					...
					"languageId": "gdscript",
					"version": version,
					"text": _get_code_text(_inline_code),
				},
			},
		})
		return
...
func _inline_send_message(message: Dictionary) -> void:
	var body := JSON.stringify(message)
	var packet := "Content-Length: %d\r\n\r\n%s" % [body.to_utf8_buffer().size(), body]
	var err := _inline_tcp.put_data(packet.to_utf8_buffer())
	if err != OK:
		print("Inline Variable: failed sending request, error %d." % err)


func _inline_read_available_messages() -> void:
	var available := _inline_tcp.get_available_bytes()
	if available <= 0:
		return

	var read_result := _inline_tcp.get_data(available)
	if read_result[0] != OK:
		print("Inline Variable: failed reading response.")
		return

	_inline_read_buffer.append_array(read_result[1])

	while true:
		var body := _try_extract_lsp_body(_inline_read_buffer)
		if body.is_empty():
			return

		_inline_read_buffer = _consume_lsp_message(_inline_read_buffer)
		var message = JSON.parse_string(body)
		if typeof(message) == TYPE_DICTIONARY:
			_inline_handle_message(message)


func _inline_handle_message(message: Dictionary) -> void:
	if not message.has("id"):
		return

	var id := _normalize_response_id(message["id"])
	if not _inline_pending_requests.has(id):
		return

	var request_kind: String = _inline_pending_requests[id]
	_inline_pending_requests.erase(id)
	...
	if request_kind == "initialize":
		...
		_inline_send_initialized_notification()
		_inline_try_send_references_request()
	elif request_kind == "references":
...
func _inline_reset_connection() -> void:
	_inline_tcp.disconnect_from_host()
	_inline_tcp = StreamPeerTCP.new()
	_inline_read_buffer.clear()
	_inline_pending_requests.clear()
	_inline_initialized = false
	_inline_last_status = StreamPeerTCP.STATUS_NONE
	_inline_opened_documents.clear()
	_inline_document_versions.clear()


func _inline_next_request_id() -> int:
	var id := _inline_next_id
	_inline_next_id += 1
	return id
