...
func next_expand_range(code_text: String, current: SmartSelectionRange) -> SmartSelectionRange:
	var candidates := _build_expansion_candidates(code_text, current)

	for candidate in candidates:
		if (
			candidate.strictly_contains(current)
	...
	if not current.equals(file_range):
		_history.record(current)
		return file_range
...
func next_shrink_range(code_text: String, current: SmartSelectionRange) -> SmartSelectionRange:
	return _history.shrink_target(current, _build_expansion_candidates(code_text, current))


func _build_expansion_candidates(code_text: String, current: SmartSelectionRange) -> Array[SmartSelectionRange]:
...
func _append_candidate(candidates: Array[SmartSelectionRange], candidate: SmartSelectionRange) -> void:
	if candidate == null:
		return
...
func _get_full_file_range(code_text: String) -> SmartSelectionRange:
	...
	return SmartSelectionRange.create(0, 0, last_line, last_line_text.length())


func _candidate_starts_after_indent_caret(
	code_text: String,
	candidate: SmartSelectionRange,
	current: SmartSelectionRange
) -> bool:
	if not current.is_zero_width():
		return false
	...
	var lines := code_text.split("\n", true)
	var line_index := current.from_line
	if line_index < 0 or line_index >= lines.size():
		return false

	var line := str(lines[line_index])
	var current_col := current.from_col
	var candidate_col := candidate.from_col
